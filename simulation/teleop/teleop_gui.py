...
class GUIBase:
    def __init__(self,
        ...
        if not use_ray_tracing:
            ...
            self.render_scene: R.Scene = scene.get_renderer_scene(
            )._internal_scene
    ...
    def create_camera(self, position, look_at_dir, right_dir, name):
        ...
        builder.set_mass_and_inertia(1e-2, Pose(np.zeros(3)),
                                     np.ones(3) * 1e-4)
        ...
        rot_mat_homo = np.stack([look_at_dir, -right_dir, up_dir, position],
                                axis=1)
    ...
    def _fetch_all_views(self, use_bgr=False) -> List[np.ndarray]:
        ...
        for cam in self.cams:
            cam.take_picture()
            rgb_tensor = torch.clamp(
                torch.utils.dlpack.from_dlpack(
                    cam.get_dl_tensor("Color"))[..., :3], 0, 1) * 255
            if use_bgr:
    ...
    def take_single_view(self, camera_name: str, use_bgr=False) -> np.ndarray:
        ...
        for cam in self.cams:
            if cam.get_name() == camera_name:
                ...
                rgb = np.clip(
                    cupy.asnumpy(cupy.from_dlpack(dlpack))[..., :3], 0,
                    1) * 255
    ...
    def render(self,
               ...
               additional_views: Optional[List[np.ndarray]] = None,
               horizontal=True):
        ...
        if (additional_views is not None
                or len(self.cams) > 0) and render_all_views:
            views = self._fetch_all_views(use_bgr=True)
            if additional_views is not None:
                views.extend(additional_views)

            if horizontal:
                pad = np.ones([views[0].shape[0], 200, 3],
                              dtype=np.uint8) * 255
            else:
                pad = np.ones([200, views[0].shape[1], 3],
                              dtype=np.uint8) * 255
            ...
            target_shape = (int(target_shape[1] * self.window_scale),
    ...
    def add_sphere_visual(self,
        ...
        if label not in self.sphere_model:
            ...
            material = self.context.create_material(emission=np.zeros(4),
                                                    base_color=rgba,
                                                    specular=0.4,
                                                    metallic=0,
                                                    roughness=0.1)
            self.sphere_model[label] = self.context.create_model([mesh],
                                                                 [material])
