...
from hand_teleop.env.sim_env.extrinsic_env import LabExtrinsicRelocateEnv
...
class ExtrinsicRelocateRLEnv(LabExtrinsicRelocateEnv, RobotEnv):
    ...
    def get_reward(self, action):
        ...
        if is_contact:
            ...
            lift = self.object_pose.p[2] - self.init_obj_pos.p[2]
            if lift > 0.03:

                reward -= np.clip(100 * (lift), 0, 10)
            # else:
            #     reward += 10
            ...
            if dist_reward < 0:
                ...
                reward -= abs(dist_reward * 10)
            else:
                reward += abs(dist_reward * 10)

            angles_reward = 1 - theta_to_target  #**0.4  # Positive reward [0, 1]

            if angles_reward < 0:

                reward -= abs(angles_reward * 40)
            else:
                reward += abs(angles_reward * 40)
    ...
    def reset(self,
        ...
        if self.object_category in [
                "pen", "assembling_kits", "spoon", "kitchen", "USB"
        ]:
            for i in range(20):
    ...
    def angles_compare(self, target_quat, current_quat):
        ...
        ee_angles = r.as_euler('yxz', degrees=True)[0]

        if ee_angles < 0:
            ee_angles = 360 + ee_angles
        self.ee_angles = ee_angles
        ...
        if self.index == 2 or self.index == 3:
            ...
            # current_angles = current_angles + 180 - ee_angles
            if current_angles < 0:
                current_angles += 360

            target_obj_angles = 180 - (self.target_angles - (180 - ee_angles))

            r = R.from_euler('xyz', [0, -target_obj_angles, 0], degrees=True)
            orientation = r.as_quat()[[3, 0, 1, 2]]  #x,y,z,w
            position = self.target_pose.p
            position[2] += 0.05
            position[0] = 0.1
    ...
    def is_done(self):
        ...
        return (
            (self.manipulated_object.get_pose().p[2] <
             self.object_length[0] + 0.02)
    ...
    def reset_robot(self, height=None, floor_height=None):
        ...
        elif self.object_category in ["knife"]:
            self.task_index = 7
            gripper_joints = self.robot.get_active_joints()[-6:]
            for joints in gripper_joints:
                joints.set_limits([[0.60, joints.get_limits()[0, 1]]])
        ...
        if self.object_category in ["pen", "knife"]:
            ...
            random_ee_factor = self.np_random.uniform(low=0.05,
                                                      high=0.10,
                                                      size=1)[0]
            ...
            palm_pose[0] += -lab.ROBOT2BASE.p[0] - 0.20 * np.sin(
                np.pi * random_ee_factor)
            palm_pose[2] = self.object_height + 0.178 + 0.05 - (
                0.17 - 0.17 * np.cos(np.pi * random_ee_factor) * 1
            ) + 2 * self.floor_height  #self.np_random.uniform(low=0.025, high=0.05, size=1)[0】
            ...
            palm_pose[0] += -lab.ROBOT2BASE.p[0] - 0.21 * np.sin(
                np.pi * random_ee_factor)
            palm_pose[2] = self.object_height + 0.16 + 0.05 - (
                0.17 - 0.17 * np.cos(np.pi * random_ee_factor) * 1
            ) + 2 * self.floor_height  #self.np_random.uniform(low=0.025, high=0.05, size=1)[0]
            ...
            palm_pose[2] = self.manipulated_object.get_pose(
            ).p[2] + 0.05 + 0.135
            ...
            if self.index == 3:
                ...
                random_ee_factor = self.np_random.uniform(
                    low=0.05, high=0.20, size=1)[0] * 1
                r = R.from_euler('xyz',
                                 [0, 180 - 180 * random_ee_factor * 1, 0],
                                 degrees=True)
                palm_pose[0] += -lab.ROBOT2BASE.p[0] - 0.20 * np.sin(
                    np.pi * random_ee_factor)
                palm_pose[2] = self.object_height + 0.178 + 0.01 - (
                    0.17 - 0.17 * np.cos(np.pi * random_ee_factor) * 1
                ) + 2 * self.floor_height  #self.np_random.uniform(low=0.025, high=0.05, size=1)[0]
...
def main_env():
    ...
    env = ExtrinsicRelocateRLEnv(
        use_gui=True,
        robot_name="xarm7_gripper_down",
        object_name="any_train",  #103733
        object_category="spoon",  #assembling_kits #02946921
        frame_skip=10,
