...
from hand_teleop.env.rl_env.robot_stable_gripper import RobotEnv
from hand_teleop.env.sim_env.stable_env import StablePlaceEnv
...
class StablePlaceRLEnv(StablePlaceEnv, RobotEnv):
    ...
    def __init__(self,
        ...
        if object_category in ["pen", "USB", "knife"]:
            self.task_index = 7
        elif object_category in ["spoon"]:
            self.task_index = 8
    ...
    def update_cached_state(self):
        ...
        if self.object_category.isnumeric():
            self.contact_boolean, force = self.check_actor_pair_contacts(
                check_contact_links,
                self.manipulated_object,
    ...
    def get_oracle_state(self):
        ...
        if self.use_buffer:
            ...
            return np.concatenate([
                ...
                # self.target_pose.q,
                [
                    self.target_angles / 180 * np.pi,
                    self.current_angles / 180 * np.pi
                ]
            ])
    ...
    def get_robot_state(self):
        ...
        return np.concatenate(
            [robot_qpos_vec, [self.target_angles / 180 * np.pi]])

    def eval_angles(self, theta):
        if self.pre_theta < 0:
            error_percentage = abs(theta) / abs(self.pre_theta)
        else:
            if theta > 0:
                error_percentage = abs(theta) / self.pre_theta
            else:
                error_percentage = (abs(theta) +
                                    self.pre_theta) / self.pre_theta
        if np.sum(self.robot_object_contact[:2]) >= 1 and np.linalg.norm(
                self.target_in_object) < 0.1:
    ...
    #     current_angles = self.target_angles - theta

    #     error_percentage = abs(theta) / self.target_angles
    ...
    def get_reward(self, action):
        ...
        if self.object_category.isnumeric():
            is_contact = np.sum(self.robot_object_contact[:2]) >= 1

        # theta_x, theta_y, theta_z = quat_delta(self.object_pose.q,
        #                                        self.target_pose.q)

        contact_table, force = self.check_actor_pair_contacts(
            [self.box], self.manipulated_object, impulse_threshold=1e-3)

        self.pre_contact_table = contact_table

        theta = self.angles_compare(self.target_pose.q, self.object_pose.q)

        if self.pre_theta > 0:
            if theta < 0:
                theta = self.pre_theta + abs(theta)

            theta_to_target = theta / self.pre_theta

        else:
            if theta < 0:
                theta_to_target = abs(theta) / abs(self.pre_theta)
            else:

                theta_to_target = abs(theta) / abs(self.pre_theta) + 1

        self.theta = theta
        self.error_percentage = self.eval_angles(theta)

        # if self.current_step > 180:
        ...
        if is_contact:
            ...
            dist_to_target = dist_to_target / 0.05
            # dist_to_target = 1 if dist_to_target > 1 else dist_to_target

            # dist_to_target = abs(self.target_in_object[2]) / abs(
            #     self.pre_target_in_object[2])
            dist_reward = np.clip(1 - dist_to_target, -1,
                                  1)  #**0.4  # Positive reward [0, 1]

            # if abs(theta) < 10:

            self.contact_table = contact_table

            if np.max(contact_table) > 0:
                reward += 10

            if dist_reward < 0:

                reward -= abs(dist_reward * 15)
            else:
                reward += abs(dist_reward * 15)

            angles_reward = np.clip(1 - theta_to_target, -1,
                                    1)  #**0.4  # Positive reward [0, 1]
            # print(angles_reward, theta_to_target, theta, self.pre_theta)
            # if np.max(contact_table) > 0:

            if angles_reward < 0:

                reward -= abs(angles_reward * 20)
            else:
                reward += abs(angles_reward * 20)
        ...
        return (reward + action_penalty + controller_penalty) / 10 + np.sum(
            ...
            [1, 1, -3]) / 2 * 0.15  #- palm_self.contact_boolean - np.sum(
    ...
    def reset(self,
        ...
        self.pre_target_in_object = self.target_in_object
        ...
        self.pre_theta = self.rotation_factor * 90
        ...
        for i in range(20):
            ...
            # else:
            #     qpos = self.robot.get_qpos()[-6:]

            #     for joints in gripper_joints:
            #         joints.set_limits([[(np.min(qpos) + np.max(qpos)) / 2,
            #                             joints.get_limits()[0, 1]]])
        ...
        self.first_theta = self.current_angles
    ...
    def angles_compare(self, target_quat, current_quat):
        ...
        r = R.from_quat(target_quat[[1, 2, 3, 0]])
        self.target_angles = r.as_euler('xyz', degrees=True)[1]

        r = R.from_quat(current_quat[[1, 2, 3, 0]])
        current_angles = r.as_euler('yzx', degrees=True)[0]
    ...
    def is_done(self):
        ...
        return (
            self.manipulated_object.get_pose().p[2] < 2 * self.floor_height
    ...
    def reset_robot(self, height=None, floor_height=None):

        if self.object_category in ["pen", "USB"]:
            self.task_index = 7
            gripper_joints = self.robot.get_active_joints()[-6:]
            for joints in gripper_joints:
                joints.set_limits([[0.60, joints.get_limits()[0, 1]]])

        elif self.object_category in ["breaking"]:
            self.task_index = 7
            gripper_joints = self.robot.get_active_joints()[-6:]
            for joints in gripper_joints:
                joints.set_limits([[0.30, joints.get_limits()[0, 1]]])

        elif self.object_category in ["box"]:
            self.task_index = 7
            gripper_joints = self.robot.get_active_joints()[-6:]
            for joints in gripper_joints:
                joints.set_limits([[0.55, joints.get_limits()[0, 1]]])
        elif self.object_category in ["breaking_bottle"]:
            self.task_index = 7
            gripper_joints = self.robot.get_active_joints()[-6:]
            for joints in gripper_joints:
                joints.set_limits([[0.80, joints.get_limits()[0, 1]]])
        elif self.object_category in ["bucket"]:
            self.task_index = 7
            gripper_joints = self.robot.get_active_joints()[-6:]
            for joints in gripper_joints:
                joints.set_limits([[0.30, joints.get_limits()[0, 1]]])
        ...
        #         "pen", "bottle", "breaking", "breaking_bottle", "box"
        # ]:

        palm_pose = self.manipulated_object.get_pose().p

        if self.rotation_factor < 0:
            random_ee_factor = self.np_random.uniform(
                low=abs(self.rotation_factor) / 2, high=0.4, size=1)[0]

        else:
            random_ee_factor = self.np_random.uniform(low=0, high=0.11,
                                                      size=1)[0]

        r = R.from_euler('xyz', [0, 180 - 180 * random_ee_factor * 1, 0],
                         degrees=True)

        # palm_pose[0] += -lab.ROBOT2BASE.p[0] - 0.21 * np.sin(
        #     np.pi * random_ee_factor)
        palm_pose[0] = -lab.ROBOT2BASE.p[0] + self.manipulated_object.get_pose(
        ).p[0] - (0.19 * np.sin(np.pi * random_ee_factor)) * 1

        palm_pose[2] = self.manipulated_object.get_pose().p[2] + (
            0.19 * np.cos(np.pi * random_ee_factor)) * 1
        # palm_pose[2] = self.object_height * np.cos(
        #     self.rotation_factor * np.pi / 2) + 2 * self.floor_height + (
        #         0.19 * np.cos(np.pi * random_ee_factor)) * 1
...
def main_env():
    ...
    env = StablePlaceRLEnv(
        use_gui=True,
        robot_name="xarm7_gripper_down",
        object_name="any_train",  #103733
