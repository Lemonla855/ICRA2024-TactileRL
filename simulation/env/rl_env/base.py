...
class BaseRLEnv(BaseSimulationEnv, gym.Env):
    ...
    def setup(self, robot_name):
        ...
        #     velocity_limit = np.array([0.15] * 4 + [0.15] * 3 +
        #                               [1.0] * hand_dof)  #slow down
        ...
        urdf_file = "assets/robot/xarm7_gripper/xarm7_with_gripper.urdf"
        # urdf_file = "/media/lme/data/cvpr-tutorial-2022/assets/robotiq_2f_85_gripper_visualization/robotiq_85_original.urdf"
        ...
        # robot = load_robot(scene, "xarm7_gripper_front", disable_self_collision=False)
        self.robot.set_pose(
            sapien.Pose(p=np.array([0.0, 0, 0]), q=np.array([1, 0, 0, 0])))
    ...
    def arm_sim_step(self, action: np.ndarray):
        ...
        target_qpos = np.concatenate([arm_qpos, hand_qpos * 0])
