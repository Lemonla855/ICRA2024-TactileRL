...
def process_relocate_pc(cloud: np.ndarray,
                        camera_pose: np.ndarray,
                        num_points: int,
                        np_random: np.random.RandomState,
                        segmentation=None,
                        floor_height=0,
                        upper_bound=0) -> np.ndarray:
    ...
    within_bound = np.nonzero(
        np.logical_and.reduce(
            (within_bound_x, within_bound_y, within_bound_z)))[0]
...
def process_extrinsic_pc(cloud: np.ndarray,
                         camera_pose: np.ndarray,
                         num_points: int,
                         np_random: np.random.RandomState,
                         segmentation=None,
                         floor_height=0,
                         upper_bound=0,
                         noise_table=False) -> np.ndarray:
    """ pc: nxm, camera_pose: 4x4 """
    pc = cloud[..., :3]
    pc = pc @ camera_pose[:3, :3].T + camera_pose[:3, 3]
    bound = lab.RELOCATE_BOUND.copy()
    bound[4] += 2 * floor_height
...
def process_extrinsic_pc_noise(cloud: np.ndarray,
                               camera_pose: np.ndarray,
                               num_points: int,
                               np_random: np.random.RandomState,
                               segmentation=None,
    ...
    if num_index < num_points:
        ...
        multiplicative_noise = 1 + np_random.randn(
            num_index)[:, None] * 0.01 * noise_level  # (num_index, 1)
        ...
        indices = within_bound[np_random.permutation(num_index)[:num_points]]
        multiplicative_noise = 1 + np_random.randn(
            num_points)[:, None] * 0.01 * noise_level  # (n, 1)
...
def process_relocate_pc_noise(cloud: np.ndarray,
                              camera_pose: np.ndarray,
                              num_points: int,
                              np_random: np.random.RandomState,
                              segmentation=None,
                              noise_level=1) -> np.ndarray:
    """ pc: nxm, camera_pose: 4x4 """
    if segmentation is not None:
        raise NotImplementedError
...
def add_gaussian_noise(cloud: np.ndarray,
                       np_random: np.random.RandomState,
                       noise_level=1):
    # cloud is (n, 3)
