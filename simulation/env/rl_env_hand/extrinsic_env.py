...
from hand_teleop.env.sim_env.extrinsic_env import RelocateEnv, LabExtrinsicRelocateEnv
...
class ExtrinsicRelocateRLEnv(LabExtrinsicRelocateEnv, BaseRLEnv):
    ...
    def create_collision(self):
        ...
        # # Top
        obj_pose = sapien.Pose(np.array([0.0, 0.1, 0.06]),
                               [0.707, 0, 0, 0.707])
        obj_material = self.scene.create_physical_material(1, 0.5, 0.01)

        obj_half_size = [0.002, 0.4, 0.12]
    ...
    def get_reward(self, action):
        ...
        diff_angle = self.angle_diff(self.object_pose.q, self.target_pose.q)

        if is_contact and self.robot_object_contact[0] == 1:

            target_obj_dist = np.linalg.norm(self.target_in_object[:2])

            reward += 1.0 / (0.04 + target_obj_dist)
            theta = self.target_in_object_angle[0]
            reward += 4.0 / (0.4 + diff_angle) * self.rotation_reward_weight

            # if (np.pi / 2 - diff_angle) > 0:  # right rotation
            #     reward += 4.0 / (0.4 +
            #                      diff_angle) * self.rotation_reward_weight
            # else:  #wrong rotation
            #     reward -= 4.0 / (
            #         0.4 + (np.pi - diff_angle)) * self.rotation_reward_weight
        ...
        return (reward + action_penalty + controller_penalty) / 10 + np.sum(
            self.robot_object_contact *
            [2, 1, 1, 1, 0]) / 5  #- palm_contact_boolean - np.sum(
...
def main_env():
    ...
    env = ExtrinsicRelocateRLEnv(use_gui=True,
                                 robot_name="xarm6_allegro_sfr_down",
                                 object_name="box",
                                 object_category="extrinsic",
                                 frame_skip=10,
