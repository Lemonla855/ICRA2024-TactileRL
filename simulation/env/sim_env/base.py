...
class BaseSimulationEnv(object):
        ...
        if mode == 'human':
            if self.viewer is None:
                ...
                self.viewer.set_camera_rpy(
                    r=0, p=0, y=-np.pi)  # change the viewer direction
    ...
    def check_actor_pair_contact(self,
                                 actor1: sapien.Actor,
                                 actor2: sapien.Actor,
                                 impulse_threshold=1e-2) -> bool:
    ...
    def check_actor_pair_contacts(
        ...
        contact_force = np.zeros(4)

        if not isinstance(actor2,
                          sapien.pysapien.Articulation) and not isinstance(
                              actor2, list):
            actor_set1 = set(actors1)
            contact_buffer = np.zeros(len(actors1))
            contact_force = np.zeros(len(actors1))

            for contact in self.scene.get_contacts():
                contact_actors = {contact.actor0, contact.actor1}

                if len(actor_set1
                       & contact_actors) > 0 and actor2 in contact_actors:
                    impulse = [point.impulse for point in contact.points]
                    # for point in contact.points:
                    #     if point.separation < -0.001:
                    ...
                    contact_force[actor_index] += np.sum(np.abs(impulse))
            ...
            actor_set1 = set(actors1)
            contact_buffer = np.zeros(len(actors1))
            if isinstance(actor2, list):
                actor_set2 = set(actor2)

            # for partnet object, need to get multiple links for contact checking
            elif isinstance(actor2, sapien.pysapien.Articulation):
                actor_set2 = set(actor2.get_links())

            id_list = []
            for link in actor_set2:
            ...
            for contact in self.scene.get_contacts():
                ...
                if len(actor_set1
                    ...
                    for remove_actor in contact_actors:  #remove the actor in the actor2
                        if remove_actor in actor_set2:
                            contact_actors.remove(remove_actor)
                            break

                    actor_index = actors1.index(contact_actors.pop())
                    contact_buffer[actor_index] = 1

        return contact_buffer, contact_force
    ...
    def create_table(self,
        ...
        if self.renderer and not self.no_rgb:
            ...
            if self.use_gui or self.use_visual_obs:
                builder.add_visual_from_file(str(table_cube_path),
                                             pose=top_pose,
                                             material=table_visual_material,
                                             scale=table_half_size,
                                             name="surface")
                builder.add_box_visual(pose=sapien.Pose([x, y, leg_height]),
                                       half_size=leg_size,
                                       material=table_visual_material,
                                       name="leg0")
                builder.add_box_visual(pose=sapien.Pose([x, -y, leg_height]),
    ...
    def create_camera(self,
        ...
        if mount_actor_name is not None:
            mount = [
                actor for actor in self.scene.get_all_actors()
            ...
            mount = mount[0]
            cam = self.scene.add_mounted_camera(name,
                                                mount,
                                                Pose(),
                                                width=resolution[0],
                                                height=resolution[1],
            ...
            right_dir = right_dir - np.sum(right_dir * look_at_dir).astype(
                np.float64) * look_at_dir
            right_dir = right_dir / np.linalg.norm(right_dir)
            up_dir = np.cross(look_at_dir, -right_dir)
            rot_mat_homo = np.stack(
                [look_at_dir, -right_dir, up_dir, position], axis=1)
            pose_mat = np.concatenate([rot_mat_homo, np.array([[0, 0, 0, 1]])])
            pose_cam = sapien.Pose.from_transformation_matrix(pose_mat)
            cam = self.scene.add_camera(name,
                                        width=resolution[0],
                                        height=resolution[1],
                                        fovy=fov,
                                        near=0.1,
    ...
    def create_camera_from_pose(self, pose: sapien.Pose, name: str,
                                resolution: Sequence[Union[float,
                                                           int]], fov: float):
        if not len(resolution) == 2:
            raise ValueError(
                f"Resolution should be a 2d array, but now {len(resolution)} is given."
        ...
        cam.set_local_pose(
            pose_cam
        )  #[0.412396, -0.355242, 0.454189], [-0.315856, 0.212542, -0.0472674, -0.923486]
    ...
    def setup_camera_from_config(self, config: Dict[str, Dict]):
        ...
        for cam_name, cfg in config.items():
            if cam_name in self.cameras.keys():
                raise ValueError(
                    f"Camera {cam_name} already exists in the environment")

            if "mount_actor_name" in cfg:
                self.create_camera(None,
                                   None,
                                   None,
                                   name=cam_name,
                ...
                if "position" in cfg:
                    self.create_camera(cfg["position"],
                                       cfg["look_at_dir"],
                                       cfg["right_dir"],
                                       cam_name,
                                       resolution=cfg["resolution"],
                    ...
                    self.create_camera_from_pose(cfg["pose"],
                                                 cam_name,
                                                 resolution=cfg["resolution"],
                                                 fov=cfg["fov"])
                else:
                    raise ValueError(
                        f"Camera {cam_name} has no position or pose.")
        # near, far = 0.0001, 100
