...
def get_engine_and_renderer(use_gui=True,
    ...
    if use_ray_tracing:
        ...
        if need_renderer:
            ...
            if render_ssp:
                ...
                sapien.render_config.rt_samples_per_pixel = 1024  # change to 256 for less noise
                sapien.render_config.rt_use_denoiser = False  # change to True for OptiX denoiser

            _renderer = sapien.VulkanRenderer(
...
def add_default_scene_light(scene: sapien.Scene,
    ...
    # scene.add_directional_light(np.array([-1, -1, -1]),
    #                             np.array([0.5, 0.5, 0.5]),
    #                             shadow=cast_shadow)
    # scene.add_directional_light([0, 0, -1], [0.9, 0.8, 0.8], shadow=False)
    # scene.add_spot_light(np.array([0, 0, 1.5]),
    ...
    # scene.add_directional_light([0, 0, -2], [0.5, 0.5, 0.5])
    ...
    physical_material = scene.create_physical_material(1.0, 1.0, 0.0)
    ...
    scene.add_ground(-1, render_material=render_material)
    # if add_ground:
