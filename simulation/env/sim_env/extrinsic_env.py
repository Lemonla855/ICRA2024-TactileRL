...
from hand_teleop.utils.partnet_utils import load_partnet_actor, PARTNET_ORIENTATION, PEN_ANYTRAIN2, PEN_ANYTRAIN_NOVEL, SPOON_NOVEL_ANYTRAIN, ANY_KIND, ALL_KIND, USB_ANYTRAIN_NOVEL, USB_ANYTRAIN, load_partnet_arti2actor, load_knife_actor, KNIFE_ANYTRAIN
...
class LabExtrinsicRelocateEnv(BaseSimulationEnv):
    ...
    def create_pan(self):
        ...
        self.floor_height = self.np_random.uniform(low=0.001,
                                                   high=0.10,
                                                   size=1)[0]
    ...
    def load_object(self, object_name):
        ...
        if self.object_category.lower() == "spoon":
            ...
            # set_entity_color([manipulated_object],
            #                  np.array([208, 245, 190, 255]) / 255)
            # target_object = None
            if self.render_mesh:
            ...
            if self.all_object_category is not None:
                if self.novel:
                    self.obj_index += len(PEN_ANYTRAIN_NOVEL)
                else:
                    self.obj_index += len(PEN_ANYTRAIN2)

        elif self.object_category.lower() == "kitchen":
            ...
            manipulated_object, self.scale, object_height, self.object_length = load_knife_actor(
                self.scene,
                self.object_category,
                model_name=object_name,
                height=height)

            target_object, self.scale, object_height, _ = load_knife_actor(
                self.scene,
                self.object_category,
                model_name=object_name,
                visual_only=True,
            ...
            height = self.np_random.uniform(low=0.13, high=0.22, size=1)[0]
    ...
    def generate_random_object_pose(self, randomness_scale):
        ...
        if self.object_category in ["spoon"]:
            ...
            if self.index == 0:
                ...
                self.sign_distance = 1
                self.rotation_factor = self.np_random.uniform(low=-0.40,
                                                              high=0.10)

                position = np.array([
                    0, 0, (self.object_height) *
                    np.cos(self.sign_distance * self.rotation_factor * 90 /
                           180 * np.pi) + 2 * self.floor_height
                ])
            elif self.index == 3:
                self.sign_distance = 1
                self.rotation_factor = self.np_random.uniform(low=-0.40,
            ...
            r = R.from_euler(
                'xyz', [0, self.sign_distance * self.rotation_factor * 90, 0],
                degrees=True)
            ...
            orientation = r.as_quat()[[3, 0, 1, 2]]
            position = np.array(
                [0, 0, self.object_height + 2 * self.floor_height])
    ...
    def generate_random_target_pose(self, randomness_scale):
        ...
        position = np.array([0, 0, self.object_length[2]])
        ...
        if self.object_category in ["spoon", "kitchen"]:
            ...
            if self.index == 0:
                ...
                position = np.array([
                    ...
                    self.object_height * np.cos(
                        ...
                        np.pi) + 2 * self.floor_height
                ])
            elif self.index == 1:
                r = R.from_euler('xyz', [0, 0, 0], degrees=True)
                position = np.array([
                    -self.sign_distance * self.object_height * 0.5, 0,
                    self.object_height + 2 * self.floor_height
                ])

            elif self.index == 2 or self.index == 3:
                rotation_factor = self.np_random.uniform(low=0.6, high=0.99)
                r = R.from_euler('xyz', [0, -90 * rotation_factor, 0],
                                 degrees=True)
                position = np.array([
            ...
            if self.index == 2:
                rotation_factor = self.np_random.uniform(low=0.3, high=0.99)

            r = R.from_euler('xyz', [0, -90 * rotation_factor, 0],
                             degrees=True)
            orientation = r.as_quat()[[3, 0, 1, 2]]  #x,y,z,w
            position = np.array([
                -self.object_height * 0.5 + 0.1, 0,
                self.object_height * np.cos(
                    (90 * rotation_factor * self.sign_distance) / 180 * np.pi)
                + 2 * self.floor_height
            ])
