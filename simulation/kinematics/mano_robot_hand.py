...
from transforms3d.quaternions import mat2quat, qmult, quat2axangle
...
class MANORobotHand:
    def __init__(self,
        ...
        self.finger_viz_mat_kwargs = dict(specular=0.5,
                                          metallic=0.0,
                                          roughness=0.1,
                                          base_color=(0.1, 0.1, 0.1, 1))
        self.tip_viz_mat_kwargs = dict(specular=0.07,
                                       metallic=0.2,
                                       roughness=0.5,
                                       base_color=(0.9, 0.9, 0.9, 1))

        # Create robot
        self.use_full_dof = full_dof
        ...
        if full_dof:
            self.robot = create_mano_full_dof_robot_hand(
                scene,
                renderer,
                init_joint_pos,
                free_root=free_root,
            ...
            # TODO:
            self.robot = create_mano_articulated_robot_hand(
                scene,
                renderer,
                init_joint_pos,
                free_root=free_root,
                finger_viz_mat=finger_viz_mat,
                tip_viz_mat=tip_viz_mat)
        self.robot.set_qpos(
            np.zeros(self.robot.dof
                     ))  # Set qpos to update the articulation architecture
        ...
        for index, link in enumerate(self.robot.get_links()):
            ...
            if part_name not in self.part_names:
                continue
            self.link_index_dict[self.part_names.index(part_name)].append(
                index)
    ...
    def control_robot(self,
        ...
        if self.position_control:
            ...
            if lock_indices:
                ...
                drive_target[joint_indices] = self.robot.get_drive_target(
                )[joint_indices]
    ...
    def check_contact_finger(self, actors: List[sapien.Actor]) -> List[int]:
        ...
        for contact in self.scene.get_contacts():
            ...
            if len(link_set & contact_actors) > 0 and len(
                    actor_set & contact_actors) > 0:
                ...
                for geometry in link.get_visual_bodies():
                    for shape in geometry.get_render_shapes():
                        ...
                        if index in finger_indices:
                            ...
                            mat.set_base_color(
                                self.tip_viz_mat_kwargs["base_color"])
                        else:
                            mat.set_base_color(
                                self.finger_viz_mat_kwargs["base_color"])
    ...
    def finger_tips(self):
        ...
        finger_tips = {
            link.get_name(): link
            for link in self.robot.get_links() if link.get_name() in tip_names
        }
    ...
    def finger_middles(self):
        ...
        finger_middles = {
            link.get_name(): link
            for link in self.robot.get_links()
            if link.get_name() in middle_names
        }
        return [finger_middles[name] for name in finger_middles]


def create_mano_articulated_robot_hand(scene: sapien.Scene,
                                       renderer: sapien.VulkanRenderer,
                                       joint_pos: np.ndarray,
                                       finger_viz_mat: sapien.RenderMaterial,
                                       tip_viz_mat: sapien.RenderMaterial,
                                       free_root=True):
    # Compute shape related params
    joint_pos = joint_pos - joint_pos[:1, :]
    finger_palm_width = np.abs(joint_pos[17, LITTLE_TO_THUMB_AXIS] -
                               joint_pos[5, LITTLE_TO_THUMB_AXIS]) / 3

    # Visual material

    # Build palm and four palm finger geom
    robot_builder = scene.create_articulation_builder()
    palm = _create_palm(robot_builder, free_root)

    for i in range(4):
        finger_palm_length = np.abs(joint_pos[5 + 4 * i, INVERSE_ALONG_AXIS])
        pos = np.array([
            -finger_palm_length / 2, 0,
            joint_pos[5, LITTLE_TO_THUMB_AXIS] - i * finger_palm_width
        ])
        palm.add_box_visual(pose=Pose(pos),
                            half_size=np.array([
                                finger_palm_length / 2, PALM_THICKNESS,
                                finger_palm_width / 2
                            ]),
                            material=finger_viz_mat)
    ...
    limit_finger_11 = [[0, np.pi / 2]] * 5
    limit_finger_21 = [[0, np.pi / 2]] * 5
    rot_x_to_z = euler2quat(0, -np.pi / 2, 0)
    for i in range(5):
        finger_00 = robot_builder.create_link_builder(palm)
        rot_01 = rot_from_connected_link(joint_pos[1 + 4 * i],
                                         joint_pos[2 + 4 * i])
        quat_01 = mat2quat(rot_01)

        # First finger link
        quat_y_rotate = euler2quat(0, 0, np.pi / 2)
        finger_00.set_joint_name(f"{finger_names[i]}_00_joint")
        finger_00.set_joint_properties("revolute",
                                       limits=np.array([limit_finger_00[i]]),
                                       pose_in_parent=Pose(
                                           joint_pos[1 + 4 * i],
                                           qmult(quat_01, quat_y_rotate)),
                                       pose_in_child=Pose(q=quat_y_rotate))
        finger_00.set_name(f"{finger_names[i]}_00")

        finger_01 = robot_builder.create_link_builder(finger_00)
        finger_01.set_name(f"{finger_names[i]}_01")
        finger_01.set_joint_name(f"{finger_names[i]}_01_joint")
        finger_01.set_joint_properties("revolute",
                                       limits=np.array([limit_finger_01[i]]),
                                       pose_in_parent=Pose(q=rot_x_to_z),
                                       pose_in_child=Pose(q=rot_x_to_z))
        finger_01.set_name(f"{finger_names[i]}_01")
        height_01 = np.linalg.norm(joint_pos[2 + 4 * i] - joint_pos[1 + 4 * i])
        finger_01.add_capsule_visual(pose=Pose([-height_01 / 2, 0, 0]),
                                     radius=FINGER_RADIUS,
                                     half_length=height_01 / 2,
                                     material=finger_viz_mat)

        # Second finger link
        rot_11 = rot_from_connected_link(joint_pos[2 + 4 * i],
                                         joint_pos[3 + 4 * i],
                                         parent_rotation=rot_01)
        quat_11 = mat2quat(rot_11)
        height_11 = np.linalg.norm(joint_pos[3 + 4 * i] - joint_pos[2 + 4 * i])
        finger_11 = robot_builder.create_link_builder(finger_01)
        finger_11.set_name(f"{finger_names[i]}_11")
        finger_11.set_joint_name(f"{finger_names[i]}_11_joint")
        finger_11.set_joint_properties("revolute",
                                       limits=np.array([limit_finger_11[i]]),
                                       pose_in_parent=Pose([-height_01, 0, 0],
                                                           q=qmult(
                                                               quat_11,
                                                               rot_x_to_z)),
                                       pose_in_child=Pose(q=rot_x_to_z))
        finger_11.add_capsule_visual(pose=Pose([-height_11 / 2, 0, 0]),
                                     radius=FINGER_RADIUS,
                                     half_length=height_11 / 2,
                                     material=finger_viz_mat)

        # Third finger link
        rot_21 = rot_from_connected_link(joint_pos[3 + 4 * i],
                                         joint_pos[4 + 4 * i],
                                         parent_rotation=rot_01 @ rot_11)
        quat_21 = mat2quat(rot_21)
        height_21 = np.linalg.norm(joint_pos[3 + 4 * i] - joint_pos[2 + 4 * i])
        finger_21 = robot_builder.create_link_builder(finger_11)
        finger_21.set_name(f"{finger_names[i]}_21")
        finger_21.set_joint_name(f"{finger_names[i]}_21_joint")
        finger_21.set_joint_properties("revolute",
                                       limits=np.array([limit_finger_21[i]]),
                                       pose_in_parent=Pose([-height_11, 0, 0],
                                                           q=qmult(
                                                               quat_21,
                                                               rot_x_to_z)),
                                       pose_in_child=Pose(q=rot_x_to_z))
        finger_21.add_capsule_visual(pose=Pose([-height_21 / 2, 0, 0]),
                                     radius=FINGER_RADIUS,
                                     half_length=height_21 / 2,
                                     material=tip_viz_mat)

        # Forth finger link
        height_31 = np.linalg.norm(joint_pos[4 + 4 * i] - joint_pos[3 + 4 * i])
        finger_31 = robot_builder.create_link_builder(finger_21)
        finger_31.set_name(f"{finger_names[i]}_tip")
        finger_31.set_joint_name(f"{finger_names[i]}_tip_joint")
        finger_31.set_joint_properties("fixed",
                                       limits=np.array([]),
                                       pose_in_parent=Pose([-height_31, 0, 0]),
                                       pose_in_child=Pose())
...
def create_mano_full_dof_robot_hand(scene: sapien.Scene,
                                    renderer: sapien.VulkanRenderer,
                                    joint_pos: np.ndarray,
                                    finger_viz_mat_kwargs: Dict,
                                    tip_viz_mat_kwargs: Dict,
                                    free_root=True,
                                    scale=1):
    # Compute shape related params
    joint_pos = joint_pos - joint_pos[:1, :]
    ...
    for i in range(5):
        ...
        finger_viz_mat = create_visual_material(renderer,
                                                **finger_viz_mat_kwargs)
        tip_viz_mat = create_visual_material(renderer, **tip_viz_mat_kwargs)
        # Link 0
        _, _, link0 = build_ball_joint(robot_builder,
                                       palm,
                                       name=f"{finger_name}_0",
                                       pose=Pose(joint_pos[1 + 4 * i]))
        pos_link1_in_link0 = joint_pos[2 + 4 * i] - joint_pos[1 + 4 * i]
        length_link1_in_link0 = np.linalg.norm(pos_link1_in_link0)
        quat_link1_in_link0 = mat2quat(
            rot_from_connected_link(joint_pos[1 + 4 * i],
                                    joint_pos[2 + 4 * i]))
        if use_visual:
            link0.add_capsule_visual(pose=Pose(pos_link1_in_link0 / 2,
                                               quat_link1_in_link0),
                                     radius=radius,
                                     half_length=length_link1_in_link0 / 2,
                                     material=finger_viz_mat)
        link0.add_capsule_collision(pose=Pose(pos_link1_in_link0 / 2,
                                              quat_link1_in_link0),
                                    radius=radius,
                                    half_length=length_link1_in_link0 / 2,
                                    **friction_dict)
        ...
        # Link 1
        _, _, link1 = build_ball_joint(robot_builder,
                                       link0,
                                       name=f"{finger_name}_1",
                                       pose=Pose(pos_link1_in_link0))
        pos_link2_in_link1 = joint_pos[3 + 4 * i] - joint_pos[2 + 4 * i]
        length_link2_in_link1 = np.linalg.norm(pos_link2_in_link1)
        quat_link2_in_link1 = mat2quat(
            rot_from_connected_link(joint_pos[2 + 4 * i],
                                    joint_pos[3 + 4 * i]))
        if use_visual:
            link1.add_capsule_visual(pose=Pose(pos_link2_in_link1 / 2,
                                               quat_link2_in_link1),
                                     radius=radius,
                                     half_length=length_link2_in_link1 / 2,
                                     material=finger_viz_mat)
        link1.add_capsule_collision(pose=Pose(pos_link2_in_link1 / 2,
                                              quat_link2_in_link1),
                                    radius=radius,
                                    half_length=length_link2_in_link1 / 2,
                                    **friction_dict)
        ...
        # Link 2
        _, _, link2 = build_ball_joint(robot_builder,
                                       link1,
                                       name=f"{finger_name}_2",
                                       pose=Pose(pos_link2_in_link1))
        finger_tip_middle = joint_pos[4 + 4 * i] + np.array(
            [0, -1, 0]) * FINGER_RADIUS / 2
        pos_link3_in_link2 = finger_tip_middle - joint_pos[3 + 4 * i]
        length_link3_in_link2 = np.linalg.norm(pos_link3_in_link2)
        quat_link3_in_link2 = mat2quat(
            rot_from_connected_link(joint_pos[3 + 4 * i], finger_tip_middle))
        if use_visual:
            link2.add_capsule_visual(pose=Pose(pos_link3_in_link2 / 2,
                                               quat_link3_in_link2),
                                     radius=radius,
                                     half_length=length_link3_in_link2 / 2,
                                     material=tip_viz_mat)
        link2.add_capsule_collision(pose=Pose(pos_link3_in_link2 / 2,
                                              quat_link3_in_link2),
                                    radius=radius,
                                    half_length=length_link3_in_link2 / 2,
                                    **friction_dict)
        ...
        link3.set_joint_properties("fixed",
                                   limits=np.array([]),
                                   pose_in_parent=Pose(joint_pos[4 + 4 * i] -
                                                       joint_pos[3 + 4 * i]))
...
def _create_palm(robot_builder: sapien.ArticulationBuilder,
                 free_root: bool,
                 robot_name=""):
    if free_root:
...
def main():
    ...
    from utils.scene_utils import add_mesh_to_renderer, add_line_set_to_renderer

    # SMPLX
    import smplx
    from pathlib import Path
    import torch
    hand_vertex_index = np.array([
        i for i in range(7331, 8218) if i not in [
            7452, 7453, 7454, 7455, 7457, 7458, 7460, 7461, 7462, 7497, 7498,
            7557, 7559, 7580, 7584, 7591, 7592, 7593, 7594, 7636
        ]
    ])
    wrist_idx, hand_start_idx, middle_finger_idx = 21, 21, 43
    hand_idxs = [21] + list(range(40, 55)) + list(range(
        71, 76))  # 21 for right wrist. 20 finger joints
    smplx_hand_to_panoptic = [
        0, 13, 14, 15, 16, 1, 2, 3, 17, 4, 5, 6, 18, 10, 11, 12, 19, 7, 8, 9,
        20
    ]
    smplx_model_path = Path(
        __file__
    ).parent.parent / "hand_detector" / "extra_data" / "smpl" / "SMPLX_NEUTRAL.pkl"
    smplx = smplx.create(str(smplx_model_path.resolve()),
                         model_type="smplx",
                         batch_size=1,
                         gender='neutral',
                         num_betas=10,
    ...
    def forward_smplx(pose_param):
        ...
        vertices = (vertices - joints[:, hand_start_idx:hand_start_idx +
                                      1, :]).cpu().numpy()[0] - origin
        joint_pos -= origin
    ...
    # Lighting
    render_scene: sapien.RenderScene = scene.get_renderer_scene()
    render_scene.set_ambient_light(np.array([0.6, 0.6, 0.6]))
    render_scene.add_directional_light(np.array([1, -1, -1]),
                                       np.array([1, 1, 1]))
    render_scene.add_point_light(np.array([2, 2, 2]), np.array([2, 2, 2]))
    render_scene.add_point_light(np.array([2, -2, 2]), np.array([2, 2, 2]))
    render_scene.add_point_light(np.array([-2, 0, 2]), np.array([2, 2, 2]))
    ...
    mat_hand = context.create_material(np.array([0.96, 0.75, 0.69, 1]), 0.0,
                                       0.8, 0)
    mano_robot = MANORobotHand(scene,
                               renderer,
                               joint_pos,
                               full_dof=True,
                               free_root=True,
    ...
    while not viewer.closed:
        # clear_nodes(scene, nodes)
        hand_qpos = mano_robot.compute_qpos(
            (hand_pose + smplx.right_hand_mean).cpu().numpy().flatten())
        hand_verts, joint_pos = forward_smplx(hand_pose)
        # Rendering
        obj = add_mesh_to_renderer(scene,
                                   renderer,
                                   hand_verts,
                                   faces,
                                   material=mat_hand)
        line_set = add_line_set_to_renderer(scene,
                                            renderer,
                                            joint_pos,
                                            np.stack(
                                                [parent, np.arange(21)],
        ...
        line = add_line_set_to_renderer(scene,
                                        renderer,
                                        joint_pos,
                                        np.stack(
                                            [parent, np.arange(21)], axis=1),
                                        color=np.array([1, 0, 0, 1]))
        ...
        relative_pose = (mano_robot.robot.get_links()[9].get_pose().inv() *
                         mano_robot.robot.get_links()[10].get_pose())
        print(f"Relative {relative_pose}")
        print(f"Axis angle {np.multiply(*quat2axangle(relative_pose.q))}")
        print(
            f"Shape param: {(hand_pose + smplx.right_hand_mean).cpu().numpy()[:3]}"
        )

        qpos = np.concatenate([np.zeros(6), hand_qpos])
        for i in range(5):
            mano_robot.control_robot(qpos, i, 5)
            scene.step()
            scene.update_render()
            viewer.render()
        hand_pose += torch.randn([45], dtype=torch.float32).cuda() * 0.05
