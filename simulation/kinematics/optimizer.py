...
class Optimizer:
    def __init__(self, robot: sapien.Articulation,
        ...
        self.fixed_joint_indices = np.array(
            [i for i in range(robot.dof) if i not in target_joint_index],
            dtype=int)
...
class PositionOptimizer(Optimizer):
    def __init__(self,
        ...
        for target_link_name in target_link_names:
            ...
            target_link_index.append(
                self.get_link_names().index(target_link_name))
    ...
    def _get_objective_function(self, target_pos: np.ndarray,
        ...
        def objective(x: np.ndarray, grad: np.ndarray) -> float:
            ...
            target_link_poses = [
                self.model.get_link_pose(index)
            ...
            if grad.size > 0:
                if self.use_sparse_jacobian:
                    ...
                    for i, index in enumerate(self.target_link_indices):
                        link_spatial_jacobian = self.model.compute_single_link_local_jacobian(
                            qpos, index)[:3, self.target_joint_indices]
                        link_rot = self.model.get_link_pose(
                            index).to_transformation_matrix()[:3, :3]
                    ...
                    jacobians = [
                        self.model.get_link_jacobian(
                            index, local=True)[:3, self.target_joint_indices]
                        for index in self.target_link_indices
                    ]
...
def test_position_optimizer():
    ...
    random_target = np.array([
        robot.get_links()[i].get_pose().p
        for i in optimizer.target_link_indices
    ])
    tic = time.time()
    init_qpos = np.clip(random_qpos + np.random.randn(robot.dof) * 0.1,
                        joint_limit[:, 0], joint_limit[:, 1])
    computed_qpos = optimizer.retarget(random_target,
                                       last_qpos=init_qpos[6:],
                                       fixed_qpos=random_qpos[:6])
    ...
    robot.set_qpos(qpos)
    computed_target = np.array([
        robot.get_links()[i].get_pose().p
        for i in optimizer.target_link_indices
    ])
    print("distance from target",
          np.mean(np.linalg.norm(computed_target - random_target, axis=1)))
    robot.set_qpos(init_qpos)
    init_target = np.array([
        robot.get_links()[i].get_pose().p
        for i in optimizer.target_link_indices
    ])
    print("distance from init",
          np.mean(np.linalg.norm(computed_target - init_target, axis=1)))
