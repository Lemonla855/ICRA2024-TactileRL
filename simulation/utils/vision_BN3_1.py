...
from hand_teleop.utils import ops
...
class build_image_encoder(nn.Module):
    ...
    def __init__(self,
        ...
        total_modules = []
        modules = [
            nn.Conv2d(input_shape[0], initial_num_filters,
                      **conv_layer_kwargs),
        ...
        for _ in range(layers_per_scale):
            modules.extend([
                nn.Conv2d(initial_num_filters, initial_num_filters,
                          **conv_layer_kwargs),
                nn.BatchNorm2d(initial_num_filters),
                nn.LeakyReLU(0.2, inplace=True)
            ])
        ...
        while width > output_map_width:
            # Reduce resolution:
            modules.extend([
                nn.Conv2d(num_filters, num_filters * 2, **conv_layer_kwargs),
            ...
            for _ in range(layers_per_scale):
                modules.extend([
                    nn.Conv2d(num_filters * 2, num_filters * 2,
                              **conv_layer_kwargs),
                    nn.BatchNorm2d(num_filters * 2),
                    nn.LeakyReLU(0.2, inplace=True)
                ])
...
class build_image_decoder(nn.Module):
    ...
    def __init__(self,
        ...
        total_modules = []
        modules = [
            nn.Conv2d(num_filters, num_filters, **conv_layer_kwargs),
            nn.LeakyReLU(0.2, inplace=True)
        ]
        # Expand image to initial_num_filters maps:
        ...
        for i in range(int(self.num_levels)):
            ...
            for j in range(layers_per_scale):
                if j < layers_per_scale - 1:
                    ...
                    if i < layers_per_scale - 1:
                        modules.extend([
                            nn.Conv2d(num_filters // 2, num_filters // 4,
                                      **conv_layer_kwargs),
                            nn.LeakyReLU(0.2, inplace=True)
                        ])
                    else:
...
class build_images_to_keypoints_net(nn.Module):
    ...
    def forward(self, image, pre_keypoints=None):
        ...
        if not pre_keypoints is None:
            pre_gaussian_maps = ops.keypoints_to_maps1(pre_keypoints,
                                                       sigma=3.0)
        else:
...
class build_keypoints_to_images_net(nn.Module):
    ...
    def __init__(self, cfg, image_shape):
        ...
        num_encoder_output_channels = (cfg.num_encoder_filters *
                                       image_shape[1] // cfg.heatmap_width)
        input_shape = [
            num_encoder_output_channels, cfg.heatmap_width, cfg.heatmap_width
        ]
        ...
        self.adjust_channels_of_output_image = nn.Sequential(
            nn.Conv2d(self.image_decoder.out_filters, image_shape[0],
                      **kwargs))
    ...
    def forward(self,
        ...
        # pred).
        first_frame_features, skip = self.appearance_feature_extractor(
            first_frame)
        first_frame_gaussian_maps = ops.keypoints_to_maps2(
            first_frame_keypoints,
        ...
        if predicted_gaussian_maps is None:
            gaussian_maps = ops.keypoints_to_maps2(
                keypoints,
                sigma=self.keypoint_width,
                heatmap_width=self.heatmap_width)
        else:
        ...
        combined_representation = torch.cat(
            (gaussian_maps, first_frame_features), 1)
        ...
        decoded_representation = self.image_decoder(combined_representation,
                                                    skip)
...
class build_keypoints_to_images_net_2image(nn.Module):
    ...
    def forward(self, keypoints, first_frame, first_frame_keypoints, sec_frame,
                sec_frame_keypoints):
        # Get features and maps for first frame:
        # Note that we cannot use the Gaussian maps above because the
        # first_frame_keypoints may be different than the keypoints (i.e. obs vs
        ...
        sec_frame_gaussian_maps = ops.keypoints_to_maps2(
            sec_frame_keypoints,
            sigma=self.keypoint_width,
            heatmap_width=self.heatmap_width)
        ...
        # Reconstruct image:
        combined_representation = torch.cat(
            (gaussian_maps, first_frame_gaussian_maps, sec_frame_gaussian_maps,
             first_frame_features, sec_frame_features), 1)
        #combined_representation = ops.add_coord_channels(combined_representation)
        combined_representation = self.adjust_channels_of_decoder_input(
            combined_representation)
        decoded_representation = self.image_decoder(combined_representation,
