...
def merge_o3d_meshes(
        meshes: List[o3d.geometry.TriangleMesh]) -> o3d.geometry.TriangleMesh:
    if len(meshes) < 1:
...
def render_body_to_open3d_mesh(render_body: sapien.RenderBody,
                               use_texture=True) -> o3d.geometry.TriangleMesh:
    ...
    if body_type == "mesh":
        ...
        for render_shape in render_body.get_render_shapes():
            ...
            triangle_mesh = o3d.geometry.TriangleMesh(
                Vector3dVector(vertices * scale[None, :]),
        ...
        if body_type == "box":
            ...
            render_body_mesh = o3d.geometry.TriangleMesh.create_box(
                *(half_size * 2))
            ...
            indices = np.reshape(
                render_body.get_render_shapes()[0].mesh.indices,
                [-1, 3]).astype(np.int32)
            ...
            radius = render_body.radius
            render_body_mesh = o3d.geometry.TriangleMesh.create_sphere(
                radius=radius)
        else:
        ...
        if has_material and use_texture:
            raise NotImplementedError(
                f"Only mesh type supports texture in SAPIEN.")
        else:
            vertex_color = material.base_color[:3]
...
def actor_to_open3d_mesh(actor: sapien.ActorBase,
...
def duplicate_actor_as_vulkan_nodes(actor: sapien.ActorBase,
                                    scene: sapien.Scene,
                                    use_shadow=True,
                                    opacity=None):
...
def export_scene_as_multiple_meshes(scene: sapien.Scene,
                                    directory: str,
                                    init_num=0):
    num = init_num
    ...
    for articulation in scene.get_all_articulations():
        ...
        o3d.io.write_triangle_mesh(f"{directory}/{num:0>3d}.obj",
                                   art_mesh,
                                   write_ascii=True)
        num += 1
    return num
...
def set_entity_visibility(entities: List[Union[sapien.ActorBase,
                                               sapien.ArticulationBase]],
                          visibility: float):
...
def set_entity_color(entities: List[Union[sapien.ActorBase,
                                          sapien.ArticulationBase]],
                     color: List[float]):
    if len(color) != 4:
...
def add_mesh_to_renderer(scene: sapien.Scene,
...
def add_line_set_to_renderer(scene: sapien.Scene,
                             renderer: sapien.VulkanRenderer,
                             position: np.ndarray,
...
def test_mesh_function():
    ...
    builder.add_box_visual(pose=sapien.Pose([0, 0, 1.5]),
                           half_size=[0.5, 0.5, 0.5])
    builder.add_capsule_visual(radius=0.1,
                               half_length=0.5,
