...
class EulerOptimizer:
    ...
    def init_euler_angle(self, euler_angle: np.ndarray):
        if euler_angle.shape != (3, ):
            raise ValueError(
                f"Euler angle should be in shape (3,), but given f{euler_angle.shape}"
            )
    ...
    def _get_objective_function(self, target_rot_mat: np.ndarray):
        def objective(x: np.ndarray, grad: np.ndarray) -> float:
            ...
            torch_mat = transforms.euler_angles_to_matrix(
                torch_euler, "XYZ")  # pytorch3d convention: moving frame

            torch_target_mat = torch.from_numpy(
                target_rot_mat.astype(np.float64))[None, :]
            torch_target_mat.requires_grad_(False)
            torch_last_value = torch.from_numpy(
                self.last_value.astype(np.float64))[None, :]
            torch_last_value.requires_grad_(False)

            rot_distance_cos = -transforms.so3_relative_angle(
                torch_mat, torch_target_mat, cos_angle=True)
            norm_distance = self.norm_lambda * torch.norm(torch_last_value -
                                                          torch_euler)
    ...
    def optimize(self,
        ...
        if target_rot_mat.shape != (3, 3):
            raise ValueError(
                f"Rotation matrix should be in shape (3, 3), but given f{target_rot_mat.shape}"
            )
        need_opt = True
        if self.hybrid:
            analytical_euler_angle = np.array(
                transforms3d.euler.mat2euler(target_rot_mat, "rxyz"))
            if np.linalg.norm(analytical_euler_angle -
                              self.last_value) < self.hybrid_threshold:
                need_opt = False
...
def test_euler_optimizer():
    ...
    gt_euler_seq = np.array([0.01, 0.02, 0.03
                             ])[None, ] * np.arange(seq_len)[:, None]
    ...
    for i in range(seq_len):
        ...
        analytical_euler_seq.append(
            transforms3d.euler.mat2euler(rot_mat, "rxyz"))
        if np.linalg.norm(pred_euler_seq[i] - gt_euler_seq[i]) > 0.01:
            print(
                f"Diff: {pred_euler_seq[i] - gt_euler_seq[i]}, original: {gt_euler_seq[i]}"
            )
    ...
    print(
        f"Pred-gt diff: {np.linalg.norm(pred_euler_seq - gt_euler_seq, axis=1).mean()}"
    )
    print(
        f"analytical-gt diff: {np.linalg.norm(analytical_euler_seq - gt_euler_seq, axis=1).mean()}"
    )
