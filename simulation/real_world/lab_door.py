...
def generate_lab_door(scene: sapien.Scene, renderer: sapien.VulkanRenderer,
                      wood_friction, handle_friction):
    # builder
    ...
    door_frame.add_box_collision(
        pose=sapien.Pose(
            np.array([
                left_frame_size[0] / 2,
                left_frame_size[1] / 2 + handle_y_offset + door_y_offset,
    ...
    door_frame.add_box_collision(
        pose=sapien.Pose(
            np.array([
                right_frame_size[0] / 2,
                -right_frame_size[1] / 2 - door_size[1] + handle_y_offset,
    ...
    door.set_joint_properties(
        ...
        pose_in_parent=sapien.Pose(p=np.array([0, -joint_y_offset, 0]),
                                   q=np.array([0.707, 0, -0.707, 0])),
        pose_in_child=sapien.Pose(p=np.array([0, -joint_y_offset, 0]),
                                  q=np.array([0.707, 0, -0.707, 0])),
        friction=0.1,
    )
    # Door
    door.add_box_collision(
        pose=sapien.Pose(
            np.array([
                door_size[0] / 2, -door_size[1] / 2 + handle_y_offset,
                door_size[2] / 2 + base_size[2] / 2
            ])),
    ...
    handle_pose = sapien.Pose(
        np.array([
            -mount_size[0] - handle_cylinder_size[0] / 2, 0,
            mount_size[2] / 2 + base_size[2] + mount_height
        ]))
    ...
    handle.add_box_collision(
        pose=sapien.Pose(
            np.array([
                -mount_size[0] - handle_cylinder_size[0] +
                handle_box_size[0] / 2, -handle_box_size[1] / 2,
                mount_size[2] / 2 + base_size[2] + mount_height
            ])),
