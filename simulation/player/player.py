...
class DataPlayer:
    ...
    def get_finger_tip_middle_retargeting_result(
        ...
        if np.allclose(retargeting.last_qpos,
                       np.zeros(retargeting.optimizer.dof)):
    ...
    def bake_demonstration(self,
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            if use_human_hand:
                if method == "tip_middle":
                    qpos = self.get_finger_tip_middle_retargeting_result(
                        self.human_robot_hand, retargeting, indices)
                elif method == "tip":
                    qpos = self.get_finger_tip_retargeting_result(
                        self.human_robot_hand, retargeting, indices)
                else:
...
def bake_demonstration_allegro_test():
    ...
    env.target_object.set_pose(
        sapien.Pose(target_pose_vec[:3], target_pose_vec[3:]))
