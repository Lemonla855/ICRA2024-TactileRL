...
class DataPlayer:
    ...
    def get_sim_data(self, item) -> Dict[str, Any]:
        ...
        return dict(actor=scene_actor_data,
                    articulation_drive=scene_drive_data,
                    articulation=scene_articulation_data)

    @staticmethod
    def collect_env_state(actors: List[sapien.Actor] = [],
                          articulations: List[sapien.Articulation] = []):
        data = dict(actor={}, articulation={})
        for actor in actors:
            v = actor.get_velocity()
            w = actor.get_angular_velocity()
            pose = actor.get_pose()
            actor_data = dict(velocity=v,
                              angular_velocity=w,
        ...
        for articulation in articulations:
            ...
            articulation_data = dict(qvel=qvel,
                                     qpos=qpos,
                                     pose=np.concatenate([pose.p, pose.q]))
            data["articulation"][links[0].get_id()] = articulation_data
        return data

    def get_finger_tip_retargeting_result(self,
        ...
        if use_root_local_pose:
            base_pose_inv = human_robot_hand.robot.get_links()[0].get_pose(
            ).inv()
            human_hand_joints = np.stack([(base_pose_inv * link.get_pose()).p
                                          for link in links])
        else:
            base_pose_inv = self.env.robot.get_links()[0].get_pose().inv()
            human_hand_joints = np.stack([(base_pose_inv * link.get_pose()).p
                                          for link in links])
        robot_qpos = retargeting.retarget(human_hand_joints,
    ...
    def compute_action_from_states(self, robot_qpos_prev, robot_qpos,
                                   is_contact: bool):
        ...
        if not self.env.is_robot_free:
            ...
            # print(delta_pose_spatial.p)
        else:
            delta_qpos = robot_qpos[:6] - robot_qpos_prev[:6]
            target_velocity = delta_qpos / duration
        target_velocity = np.clip((target_velocity - v_limit[:, 0]) /
                                  (v_limit[:, 1] - v_limit[:, 0]) * 2 - 1, -1,
                                  1)
        ...
        root_velocity = np.clip(filtered_velocity, -1, 1)

        # pos_gain = 2.2 if is_contact else 2
        pos_gain = 2
        limit = self.env.robot.get_qlimits()[6:]
        target_position = robot_qpos[6:]
        target_position = np.clip((target_position - limit[:, 0]) /
                                  (limit[:, 1] - limit[:, 0]) * pos_gain - 1,
                                  -1, 1)
        action = np.concatenate([root_velocity, target_position])
        return action
            ...
            if i >= 1:
                baked_data["action"].append(
                    self.compute_action_from_states(
                        baked_data["robot_qpos"][i - 1], qpos,
                        np.sum(contact_finger_index) > 0))
            if i >= 2:
                duration = self.env.frame_skip * self.scene.get_timestep()
                finger_qvel = (baked_data["action"][-1][6:] -
                               baked_data["action"][-2][6:]) / duration
...
class TableDoorEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(self,
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            # Environment state
            baked_data["state"].append(
                self.collect_env_state(articulations=[table_door]))
...
def bake_demonstration_allegro_test():
    ...
    path = Path("/home/yuzhe/data/teleop/relocate-potted_meat_can/0030.pickle")
    # path = Path("/home/yuzhe/data/teleop/table_door/0021.pickle")
    ...
    env = RelocateRLEnv(**meta_data["env_kwargs"],
                        ...
                        use_gui=True)
    env.robot.set_pose(sapien.Pose([-0.7, 0, -0.02]))
    ...
    # Retargeting
    link_names = [
        "palm_center", "link_15.0_tip", "link_3.0_tip", "link_7.0_tip",
    ...
    env.reset
    dapg_env = DAPGWrapper(env)
    actor_id = env.manipulated_object.get_id()
    # target_pose_vec = baked_data["init_door_pose"]
    # env.table_door.set_pose(sapien.Pose(target_pose_vec[:3], target_pose_vec[3:]))
    i = 0
    env.render()
    ...
    output_dir = Path(
        __file__
    ).parent / "real_trajectory" / "relocate" / env.object_name / method
    output_dir.mkdir(exist_ok=True, parents=True)
    qpos_list = []
    init_pos = baked_data["state"][0]["actor"][actor_id]["pose"][:2]
    target_pos = baked_data["state"][-1]["actor"][actor_id]["pose"][:2]
    filename = f"{int(init_pos[0] * 100)}_{int(init_pos[1] * 100)}_{int(target_pos[0] * 100)}_{int(target_pos[1] * 100)}.npy"

    env.render()
    env.viewer.toggle_pause(True)
    for obs, qpos, state, action in zip(baked_data["obs"],
                                        baked_data["robot_qpos"],
                                        baked_data["state"],
                                        baked_data["action"]):
        env.robot.set_qpos(obs[:22])
        # door_qpos = obs[22:24]
        if np.linalg.norm(qpos[:6]) > 1e-1:
            qpos_list.append(env.robot.get_qpos())
        # env.table_door.set_qpos(door_qpos)
        # dapg_env.step(action)
        # env.arm_kinematic_step(action)
        # print(env.reward())
    ...
    print(filename)
    np.save(str(file_path), np.stack(qpos_list))
