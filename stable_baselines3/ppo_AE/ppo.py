...
from stable_baselines3.common.utils import explained_variance, get_schedule_fn, update_learning_rate, get_device
from stable_baselines3.common.decoder import make_decoder
import pdb
...
class PPO(OnPolicyAlgorithm):
    ...
    def __init__(self,
        ...
        self.kl_scheduler = AdaptiveScheduler(kl_threshold=adaptive_kl,
                                              min_lr=min_lr,
                                              max_lr=max_lr,
        ...
        self.init_AE(AE_parameter)

    def init_AE(self, AE_parameter):
        obs_shape = AE_parameter['tactile_image_shape']

        self.encoder = self.policy.features_extractor.encoder

        self.decoder = make_decoder(
            AE_parameter['decoder_type'],
            obs_shape,
            AE_parameter['encoder_feature_dim'],
            AE_parameter['num_layers'],
            AE_parameter['num_filters'],
        ).to(get_device())

        # optimizer for critic encoder for reconstruction loss
        self.encoder_optimizer = th.optim.Adam(self.encoder.parameters(),
                                               lr=AE_parameter['encoder_lr'])

        # optimizer for decoder
        self.decoder_optimizer = th.optim.Adam(
            self.decoder.parameters(),
            lr=AE_parameter['decoder_lr'],
            weight_decay=AE_parameter['decoder_weight_lambda'])

        # decoder update freq
        self.decoder_update_freq = AE_parameter['decoder_update_freq']
        self.decoder_latent_lambda = AE_parameter['decoder_latent_lambda']
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if self.decoder is not None and self.num_timesteps % self.decoder_update_freq == 0:
                    self.update_decoder(
                        rollout_data.observations['tactile_image'],
                        rollout_data.observations['tactile_image'],
                    )
        ...
        self.logger.record("rollout/skipped_minibatch",
                           num_early_stopping / num_batch_update)
    ...
    def update_decoder(self, obs, target_obs):
        ...
        self.logger.record('train_ae/ae_loss', loss.cpu().detach().numpy())
