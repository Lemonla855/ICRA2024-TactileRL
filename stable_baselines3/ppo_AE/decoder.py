...
from stable_baselines3.ppo_AE.encoder import OUT_DIM
...
class PixelDecoder(nn.Module):
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32):
        ...
        self.deconvs.append(
            nn.ConvTranspose2d(num_filters,
                               obs_shape[0],
                               3,
                               stride=2,
                               output_padding=1))
    ...
    def log(self, L, step, log_freq):
        ...
        for i in range(self.num_layers):
            L.log_param('train_decoder/deconv%s' % (i + 1), self.deconvs[i],
                        step)
...
def make_decoder(decoder_type, obs_shape, feature_dim, num_layers,
                 num_filters):
