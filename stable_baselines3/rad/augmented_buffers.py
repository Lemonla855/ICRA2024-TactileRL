...
def show_stacked_imgs(obs_stack, n_img_channels=3, max_display=16):
    ...
    for i in range(1, n_stack + 1):
        ...
        grid = make_grid(
            obs_stack[:max_display,
                      (i - 1) * n_img_channels:i * n_img_channels, ...],
            4).permute(1, 2, 0).cpu().numpy()
...
def apply_image_augmentations(obs,
                              augmentations=None,
                              visualise=False,
                              n_img_channels=1):
    """
    ...
    if augmentations is not None:
        ...
        batch_min, batch_max = obs.min(axis=2, keepdim=True)[0], obs.max(
            axis=2, keepdim=True)[0]
...
class AugmentedReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        super(AugmentedReplayBuffer, self).__init__(buffer_size,
                                                    observation_space,
                                                    action_space,
                                                    device,
                                                    n_envs=n_envs)
        ...
        ), "To apply augmentations the observation space must be an image"
    ...
    def _get_samples(
        ...
        # apply augmentations
        obs_aug = apply_image_augmentations(self.to_torch(obs),
                                            self.augmentations,
                                            self.visualise_aug,
                                            self.n_img_channels)
        next_obs_aug = apply_image_augmentations(self.to_torch(next_obs),
                                                 self.augmentations,
                                                 self.visualise_aug,
                                                 self.n_img_channels)
        ...
        data = tuple((
            ...
            self.to_torch(self._normalize_reward(self.rewards[batch_inds],
                                                 env)),
        ))
...
class AugmentedRolloutBuffer(RolloutBuffer):
    ...
    def __init__(
        ...
        super(AugmentedRolloutBuffer,
              self).__init__(buffer_size, observation_space, action_space,
...
class AugmentedDictReplayBuffer(DictReplayBuffer):
    ...
    def __init__(
        ...
        super(AugmentedDictReplayBuffer,
              self).__init__(buffer_size, observation_space, action_space,
    ...
    def _get_samples(
        ...
        for key, subspace in self.observation_space.spaces.items():
            if is_image_space(subspace):
                obs[key] = apply_image_augmentations(obs[key],
                                                     self.augmentations,
                                                     self.visualise_aug,
                                                     self.n_img_channels[key])
                next_obs[key] = apply_image_augmentations(
                    next_obs[key], self.augmentations, self.visualise_aug,
                    self.n_img_channels[key])
...
class AugmentedDictRolloutBuffer(DictRolloutBuffer):
    ...
    def __init__(
        ...
        super(AugmentedDictRolloutBuffer,
              self).__init__(buffer_size, observation_space, action_space,
                             device, gae_lambda, gamma, n_envs)
