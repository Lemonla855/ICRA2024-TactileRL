...
class StackedObservations:
    ...
    def __init__(
        ...
        (
            ...
            self.stackedobs,
            self.repeat_axis,
        ) = self.compute_stacking(num_envs, n_stack, observation_space,
                                  channels_order)
        super().__init__()
    ...
    def stack_observation_space(self,
    ...
    def update(
        ...
        self.stackedobs = np.roll(self.stackedobs,
                                  shift=-stack_ax_size,
                    ...
                    if self.channels_first:
                        new_terminal = np.concatenate(
                            (self.stackedobs[i, :-stack_ax_size,
                                             ...], old_terminal),
                            axis=
                            0,  # self.stack_dimension - 1, as there is not batch dim
                        )
                    else:
                        new_terminal = np.concatenate(
                            (self.stackedobs[i, ..., :-stack_ax_size],
...
class StackedDictObservations(StackedObservations):
    ...
    def __init__(
        ...
        for key, subspace in observation_space.spaces.items():
            ...
            ) = self.compute_stacking(num_envs, n_stack, subspace,
                                      subspace_channel_order)
    ...
    def stack_observation_space(self,
        ...
        spaces_dict = {}
        for key, subspace in observation_space.spaces.items():
            low = np.repeat(subspace.low,
                            self.n_stack,
                            axis=self.repeat_axis[key])
            high = np.repeat(subspace.high,
                             self.n_stack,
                             axis=self.repeat_axis[key])
            spaces_dict[key] = spaces.Box(low=low,
                                          high=high,
                                          dtype=subspace.dtype)
        return spaces.Dict(spaces=spaces_dict)

    def reset(self, observation: Dict[str,
        ...
        for key, obs in observation.items():
            ...
            if self.channels_first[key]:
                self.stackedobs[key][:, -obs.shape[self.stack_dimension[key]]:,
                                     ...] = obs
            else:
                self.stackedobs[key][
                    ..., -obs.shape[self.stack_dimension[key]]:] = obs
        return self.stackedobs
    ...
    def update(
        ...
        for key in self.stackedobs.keys():
            ...
            if self.channels_first[key]:
                self.stackedobs[key][:, -stack_ax_size:,
                                     ...] = observations[key]
            else:
