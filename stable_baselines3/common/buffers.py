...
class ExpertBuffer(BaseBuffer):
    ...
    def __init__(self,
                 ...
                 n_envs: int = 1,
                 dataset_path=''):
        super(ExpertBuffer, self).__init__(buffer_size,
                                           observation_space,
                                           action_space,
                                           device,
                                           n_envs=n_envs)
        ...
        print('Expert buffer info:', self.observations.shape,
              self.actions.shape)
    ...
    def get_all_samples(self, env=None):
        ...
        return ReplayBufferSamples(*tuple(map(self.to_torch, data)), None,
                                   None, None)
...
class DictSSLRolloutBuffer(RolloutBuffer):
    ...
    def _get_samples(
            self,
            batch_inds: np.ndarray,
            env: Optional[VecNormalize] = None) -> DictSSLRolloutBufferSamples:
        ...
        next_actions = [
            get_next_action(self.actions, batch_inds, i) for i in range(4)
        ]
