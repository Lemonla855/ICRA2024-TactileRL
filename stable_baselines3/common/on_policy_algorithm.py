...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(self,
        ...
        self.daggger_tactile = daggger_tactile
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                # dagger
                if self.use_expert:
                    if not self.daggger_tactile:
                        _last_orcale_obs = np.array(
                            self._last_obs['oracle_state'])
                    else:
                        _last_orcale_obs = self._last_obs

                    # _last_tactile_obs = np.array(
                    #     [obs['tactile_image'] for obs in self._last_obs])
                    # self._last_obs = {}
                    # self._last_obs['oracle_state']=_last_orcale_obs
                    # self._last_obs['tactile_image'] = _last_tactile_obs
                    obs_tensor = obs_as_tensor(_last_orcale_obs, self.device)
                else:
        ...
        with th.no_grad():
            ...
            if self.use_expert:
                if not self.daggger_tactile:
                    new_obs = np.array(new_obs['oracle_state'])
