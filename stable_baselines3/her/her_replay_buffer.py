...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def __init__(
        ...
        super().__init__(buffer_size, env.observation_space, env.action_space,
                         device, env.num_envs)
    ...
    def sample(self,
        ...
        return self._sample_transitions(batch_size,
                                        maybe_vec_env=env,
                                        online_sampling=True)  # pytype: disable=bad-return-type
    ...
    def sample_goals(
        ...
        return self._buffer["next_achieved_goal"][her_episode_indices,
                                                  transitions_indices]
    ...
    def _sample_transitions(
        ...
        n_sampled_goal: Optional[int] = None,
    ) -> Union[DictReplayBufferSamples, Tuple[Dict[str, np.ndarray], Dict[
        ...
        transitions = {
            key: self._buffer[key][episode_indices,
        ...
        if online_sampling:
            ...
            return DictReplayBufferSamples(
                ...
                rewards=self.to_torch(
                    self._normalize_reward(transitions["reward"],
                                           maybe_vec_env)),
            )
        else:
    ...
    def add(
        ...
        if self.handle_timeout_termination:
            done_ = done * (1 - np.array(
                [info.get("TimeLimit.truncated", False) for info in infos]))
        else:
            done_ = done

        self._buffer["observation"][self.pos][
            self.current_idx] = obs["observation"]
        self._buffer["achieved_goal"][self.pos][
            self.current_idx] = obs["achieved_goal"]
        self._buffer["desired_goal"][self.pos][
        ...
        self._buffer["next_obs"][self.pos][
            self.current_idx] = next_obs["observation"]
        self._buffer["next_achieved_goal"][self.pos][
            self.current_idx] = next_obs["achieved_goal"]
        self._buffer["next_desired_goal"][self.pos][
