...
import gym
from stable_baselines3.ppo_rnd.rnd_network import RNDNetwork
...
class PPO(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        self.rnd_optimizer = optim.Adam(self.rnd_network.parameters(),
                                        lr=0.0001)
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # train the rnd network...
                rnd_prediction, rnd_target = self.rnd_network(
                    rollout_data.observations)
                rnd_loss = 0.5 * (
                    (rnd_prediction - rnd_target.detach())**2).mean(-1)

                # Proportion of exp used for predictor update
                mask = th.rand(len(rnd_loss)).to(self.device)
                mask = (mask < self.rnd_update_proportion).type(
                    th.FloatTensor).to(self.device)
                rnd_loss = (rnd_loss * mask).sum() / th.max(
                    mask.sum(),
                    th.Tensor([1]).to(self.device))
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if self.need_restore and self.current_restore_step < 5:
                print(
                    f"Large performance drop detected. Restore previous model."
                )
                self.set_parameters(self.last_policy_saved[0],
                                    exact_match=True,
                                    device=self.device)
                continue
            else:
    ...
    def calculate_rnd_reward(self, observation):
        rnd_prediction, rnd_target = self.rnd_network(observation)
        reward = 0.5 * ((rnd_prediction - rnd_target)**2).sum(-1)
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            # we add some tactile intrinsic reward.
            intrinsic_rewards = self.calculate_rnd_reward(
                obs_as_tensor(new_obs, self.device))
            mixed_rewards = rewards + intrinsic_rewards * self.rnd_coef
            ...
            rollout_buffer.add(self._last_obs, actions, mixed_rewards,
        ...
        #=====================tactile===========================
        callback.on_rollout_end(self.env.tactile)
        #=====================tactile===========================
