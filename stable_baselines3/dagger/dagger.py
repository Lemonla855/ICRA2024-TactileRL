...
class DAGGER(OnPolicyAlgorithm):
    """
    ...
    def __init__(
        ...
        std_coef=0.0,
        expert=None,
        expert_observation_space: Optional[Dict[str, Any]] = None,
        use_tactile=False,
    ):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if not self.use_tactile:
                    values, log_prob, entropy = self.policy.evaluate_actions(
                        rollout_data.observations["oracle_state"], actions)
                else:
                    values, log_prob, entropy = self.policy.evaluate_actions(
                ...
                if self.use_tactile:
                    a1, _, _ = self.policy(rollout_data.observations)
                else:
                    a1, _, _ = self.policy(
                        rollout_data.observations["oracle_state"])

                with th.no_grad():

                    if self.use_tactile:
                        a2 = self.expert.policy(
                            rollout_data.observations["oracle_state"])[0]

                    else:
                        a2 = self.expert.policy(rollout_data.observations)[0]
                policy_loss = F.mse_loss(a1, a2)
                ...
                #     self.policy.log_std).mean() * self.std_coef
